...
void sensorTaskFunction(void *pvParameters) {
  while(true) {
    ...
    if(pulseSensor.sawStartOfBeat()) { Serial.print("♥ Heartbeat: "); Serial.println(bpm); }
    ...
    if(gps.location.isUpdated()) { latitude=gps.location.lat(); longitude=gps.location.lng(); }

    if(asthma_status=="ALERT") { digitalWrite(RED,HIGH); digitalWrite(YELLOW,LOW); digitalWrite(GREEN,LOW); digitalWrite(BUZZER,HIGH); }
    else if(ai_risk_percent>30) { digitalWrite(RED,LOW); digitalWrite(YELLOW,HIGH); digitalWrite(GREEN,LOW); digitalWrite(BUZZER,LOW); }
    else { digitalWrite(RED,LOW); digitalWrite(YELLOW,LOW); digitalWrite(GREEN,HIGH); digitalWrite(BUZZER,LOW); }
...
<title>Hospital Asthma Monitoring</title>
<style>
...
<body>
<div class="header">Hospital Asthma Monitoring Dashboard</div>
...
Latitude: )====";
  page+=String(latitude);
  page+=R"====(<br>Longitude: )====";
  page+=String(longitude);
  page+=R"====(<br><br><a href="https://maps.google.com/?q=)====";
  page+=String(latitude)+","+String(longitude);
  page+=R"====(" target="_blank"><button class="button">Open in Google Maps</button></a><br><br><a href="/gps"><button class="button">Send Device Location</button></a></div>
<script>
function updateData(){
...
if(data.status=="SAFE"){alertBox.className="alertSafe"; alertBox.innerText="Patient Status: SAFE";}
else{alertBox.className="alertWarn"; alertBox.innerText="⚠ ASTHMA ALERT - Immediate Attention Required";}
});
}
...
void handleGPSPage(){
  String page=R"====(<html><body><h2>Sending Device Location...</h2><script>navigator.geolocation.getCurrentPosition(function(pos){fetch('/location?lat='+pos.coords.latitude+'&lon='+pos.coords.longitude);});</script></body></html>)====";
  server.send(200,"text/html",page);
}
...
void setup(){
  ...
  if(pulseSensor.begin()) Serial.println("PulseSensor initialized");
