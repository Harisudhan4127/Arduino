/* Harisudhan Baskaran
GitHub -> www.github.com/Harisudhan4127/
YouTube -> https://www.youtube.com/@harisudhantechnology
LinkedIn -> https://in.linkedin.com/in/harisudhan-baskaran
*/
...
/* ---------- FILTER FUNCTIONS ---------- */
float averageFloat(float *arr) {
  float sum = 0;
  for (int i = 0; i < FILTER_SIZE; i++) sum += arr[i];
  return sum / FILTER_SIZE;
}
int averageInt(int *arr) {
  int sum = 0;
  for (int i = 0; i < FILTER_SIZE; i++) sum += arr[i];
  return sum / FILTER_SIZE;
}
...
void sensorTaskFunction(void *pvParameters) {
  while (true) {
    ...
    if (isnan(t) || isnan(h)) issue_note = "DHT Sensor Error";
      ...
      tempBuffer[filterIndex] = t;
      humBuffer[filterIndex] = h;
    }
    ...
    // LCD Display
    lcd.clear();
    lcd.setCursor(0, 0); lcd.print("HR:"); lcd.print(bpm); lcd.print(" T:"); lcd.print(temperature);
    lcd.setCursor(0, 1); lcd.print("Air:"); lcd.print(air_quality);
